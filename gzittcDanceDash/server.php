...
class WebSocket
{
	...
	public function __construct($a, $p)
	{
		...
		$this->timer = $this->getMillisecond();
		$this->createSocket();
	...
	protected function process($user, $msg)
	{
		...
		switch($data->event){
			case 'screen':
				$this->screen = $user->socket;
				break;
			case 'mobile':
				$this->mobile = $user->socket;
				$dt['event'] = 'test_delay';
				$dt['timestamp'] = $data->timestamp;
				$this->send($this->screen, json_encode($dt));
				break;
			case 'test_delay':
				$dt['event'] = 'test_delay';
				$dt['timestamp'] = $data->timestamp;
				$this->send($this->mobile, json_encode($dt));
				break;
			case 'start_game':
				$dt['event'] = 'start_game';
				$dt['music'] = $data->music;
	...
	public function disconnect($clientSocket)
	{
		...
		{ 
			...
			$disconnect['scoket'] = (int)$clientSocket;
			$this->zb[] = $disconnect;*/
...
$web = new WebSocket('192.168.71.60','8080');
$web->run();
